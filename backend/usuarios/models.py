...
class Servicio(models.Model):
    ...
    # La relación de trabajadores se gestionará a través del Plan de Servicio
    # trabajadores = models.ManyToManyField('Trabajador', related_name='servicios', blank=True)

    def __str__(self):
        return self.nombre_serv

class PlanServicioTrabajador(models.Model):
    trabajador = models.ForeignKey(Trabajador, on_delete=models.CASCADE)
    servicio = models.ForeignKey(Servicio, on_delete=models.CASCADE)
    nombre_plan = models.CharField(max_length=200)
    descripcion_plan = models.TextField(blank=True, null=True)
    fecha_creacion = models.DateTimeField(auto_now_add=True)
    precio_ofrecido = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)
    duracion_estimada = models.CharField(max_length=50, blank=True, null=True)

    class Meta:
        unique_together = ('trabajador', 'servicio') # Un trabajador solo puede tener un plan por servicio

    def __str__(self):
        return f"{self.nombre_plan} ({self.servicio.nombre_serv} - {self.trabajador.usuario.nombre} {self.trabajador.usuario.apellido})"
